...
- I didn't use default feature importance generated by scikit-learn Random forest package to present contributions of words to the classifier since it tends to inflate the importance of continuous or high-cardinality categorical variables (2007 Strobl et). Instead, permutation importance was applied, which measured the importance of a feature as follows. It permuted the column values of a single predictor feature and then pass all test samples back through the Random Forest and recompute the accuracy. The importance of that feature is the difference between the baseline and the drop in overall accuracy caused by permuting the column.
